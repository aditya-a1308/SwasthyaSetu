...
export default function BookAppointmentPage() {
  ...
  const [doctors, setDoctors] = useState<DoctorOptionLocal[]>([])
  const [selectedDoctor, setSelectedDoctor] = useState<DoctorOptionLocal | null>(null)
  ...
  useEffect(() => {
    ...
    const loadDoctors = async () => {
      ...
      if (!isMounted) return
      setDoctors(merged as DoctorOptionLocal[])
      setLoading(false)
    }
    loadDoctors()
  ...
  useEffect(() => {
    if (!success) return
    const timer = setTimeout(() => router.push('/patient'), 3000)
    return () => clearTimeout(timer)
  }, [router, success])

  const handleFormSubmit = (e: FormEvent<HTMLFormElement>) => {
    e.preventDefault()
    if (mode === 'scheduled' && !selectedDoctor) { setError(t('appointments.select_doctor')); return }
    if (mode === 'scheduled' && !scheduledAt) { setError(t('appointments.select_date')); return }
    if (callType === 'audio' && !phone.trim()) { setError(t('appointments.phone_label')); return }
    setError('')
      ...
      <div className="flex min-h-screen items-center justify-center bg-slate-50 px-4">
        <div className="max-w-md w-full rounded-3xl bg-white p-10 text-center shadow-xl">
          ...
          <h1 className="text-2xl font-bold text-slate-800">
            {mode === 'instant' ? t('appointments.success_instant') : t('appointments.success_scheduled')}
          </h1>
          {mode === 'instant' ? (
            <p className="mt-3 text-slate-500">
              {t('appointments.success_instant_desc')} <strong>{phone}</strong>.
            </p>
          ) : meetLink ? (
            <div className="mt-4">
              <p className="text-slate-500">{t('appointments.success_video_ready')}</p>
              <a
                href={meetLink}
                target="_blank"
              ...
              >
                {t('appointments.success_join_video')}
              </a>
              <p className="mt-2 text-xs text-slate-400">{t('appointments.success_save_link')}</p>
            </div>
          ) : (
            <p className="mt-3 text-slate-500">
              {t('appointments.success_audio_desc')} <strong>{phone}</strong>.
            </p>
          )}
          <p className="mt-6 text-xs text-slate-400">{t('appointments.success_redirect')}</p>
        </div>
      </div>
    )
  }
      ...
      <div className="mx-auto max-w-2xl px-4 py-8">
        ...
        <div className="mb-6 grid grid-cols-2 gap-3 rounded-2xl bg-white p-4 shadow-sm">
          ...
          >
            ...
            <p className="font-semibold text-slate-800">{t('appointments.schedule_label')}</p>
            <p className="text-xs text-slate-500">{t('appointments.schedule_sub')}</p>
          </button>
          <button
            type="button"
          ...
          >
            <div className="text-xl mb-1">⚡</div>
            <p className="font-semibold text-slate-800">{t('appointments.instant_label')}</p>
            <p className="text-xs text-slate-500">{t('appointments.instant_sub')}</p>
          </button>
        </div>
        ...
        <form onSubmit={handleFormSubmit} className="space-y-5">
                      ...
                      >
                        <div className="flex items-center gap-3">
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <p className="text-xs text-slate-400 mt-0.5">₹{doc.consultation_fee} · {t('appointments.available_now')}</p>
                          </div>
                        </div>
                      </button>
                    )
          ...
          <section className="rounded-2xl bg-white p-5 shadow-sm">
            ...
            <div className="grid grid-cols-2 gap-3">
              {(['video', 'audio'] as const).map((type) => (
                ...
                >
                  {type === 'video' ? t('appointments.video_call') : t('appointments.audio_call')}
                </button>
              ))}
            </div>
            {callType === 'audio' && (
              <p className="mt-2 text-xs text-slate-400">{t('appointments.audio_hint')}</p>
            )}
          </section>
          ...
          {callType === 'audio' && (
            <section className="rounded-2xl bg-white p-5 shadow-sm">
              <label htmlFor="phone" className="mb-2 block text-sm font-semibold text-slate-700">
                {t('appointments.phone_label')} <span className="text-red-400">*</span>
              </label>
              <input
                id="phone"
                type="tel"
          ...
          <section className="rounded-2xl bg-white p-5 shadow-sm">
            ...
            <textarea
              ...
              placeholder={t('appointments.symptoms_placeholder')}
          ...
          <section className="rounded-2xl border border-slate-100 bg-white p-5 shadow-sm">
            <div className="flex items-center justify-between mb-3">
              ...
              <div className="flex items-center gap-1 text-xs text-slate-400">
                <span>🔒</span> {t('appointments.secured_by')}
              </div>
            </div>
            <div className="space-y-2 text-sm">
              <div className="flex justify-between text-slate-600">
                <span>{t('appointments.consultation_fee_label')}</span>
                <span>₹{fee}</span>
              </div>
              <div className="flex justify-between text-slate-400">
                <span>{t('appointments.platform_fee')}</span>
                <span className="text-emerald-600">₹0</span>
              </div>
              <div className="flex justify-between text-slate-400">
                <span>{t('appointments.gst')}</span>
            ...
            <p className="mt-2 text-xs text-slate-400">{t('appointments.demo_note')}</p>
          </section>
          ...
          >
            {submitting ? t('appointments.processing') : fee === 0 ? t('appointments.confirm_book') : `${t('appointments.proceed_pay')} ₹${fee + Math.round(fee * 0.18)}`}
          </button>
        </form>
      </div>
          ...
          <div className="w-full max-w-md rounded-t-3xl bg-white sm:rounded-3xl shadow-2xl overflow-hidden max-h-[90vh] flex flex-col">
            ...
            <div className="flex items-center justify-between border-b border-slate-100 px-6 py-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-xs text-slate-500">{t('app_name')} · {t('payment.secure_checkout')}</p>
            ...
            <div className="flex border-b border-slate-100 overflow-x-auto">
              {([
                ...
                { id: 'netbanking', label: t('payment.netbanking') },
                { id: 'wallet', label: t('payment.wallets') },
            ...
            <div className="flex-1 overflow-y-auto p-6">
              {payMethod === 'upi' && (
                <div className="space-y-4">
                  ...
                  <div className="relative">
                    ...
                    <div className="relative flex justify-center"><span className="bg-white px-3 text-xs text-slate-400">{t('payment.or_enter_upi')}</span></div>
                  </div>
                  <input
                    type="text"
                    value={upiId}
              ...
              {payMethod === 'card' && (
                <div className="space-y-4">
                  <div>
                    <label className="mb-1 block text-xs font-medium text-slate-600">{t('payment.card_number')}</label>
                    <input
                      type="text"
                      value={cardNum}
                  ...
                  <div>
                    <label className="mb-1 block text-xs font-medium text-slate-600">{t('payment.card_name')}</label>
                    <input
                      type="text"
                      value={cardName}
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      <label className="mb-1 block text-xs font-medium text-slate-600">{t('payment.expiry')}</label>
                      <input
                        type="text"
                        value={cardExpiry}
                    ...
                    <div>
                      <label className="mb-1 block text-xs font-medium text-slate-600">{t('payment.cvv')}</label>
                      <input
                        type="password"
                        value={cardCvv}
              ...
              {payMethod === 'netbanking' && (
                <div className="space-y-3">
                  <label className="block text-xs font-medium text-slate-600">{t('payment.select_bank')}</label>
            ...
            <div className="border-t border-slate-100 p-4">
              ...
              >
                {payProcessing ? (
                  ...
                  `${t('payment.pay')} ₹${fee + Math.round(fee * 0.18)}`
                )}
              </button>
